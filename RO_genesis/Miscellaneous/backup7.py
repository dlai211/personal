...
# player_pieces = {'L': 2, 'M': 3, 'S': 3}
# ai_pieces = {'L': 2, 'M': 3, 'S': 3}
player_pieces = [3, 3, 2]
ai_pieces = [3, 3, 2]
        ...
        for col in range(BOARD_COLS):
            for size in (1, 2, 3):
                if can_place_piece(row, col, size, 2, board2, ai_pieces):
                    ...
                    score = minimax(board2, 0, False, player_pieces, ai_pieces, max_depth=3)
            ...
            if col < 3 and row < 3 and piece_exists(game_pieces, {'size': dragged_size, 'player': 1, 'position': (row, col)}): # so it does NOT go out of the map
                ...
                if dragged_size == 2 and player_pieces[1] > 0:
                    ...
                    if not game_over:
                        ...
                        if BestMove:
                            game_pieces.append({'size': size_tmp, 'player': 2, 'position': (row, col)})
                            print('SQUAREEEEEEEEEEEEEEEEEEES')
                            if check_win(2):
                                game_over = True
                            player = player % 2 + 1
                    if not game_over:
