...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP and dragging:
            ...
            if col < 3 and row < 3 and NUM_ORANGE_CIRCLE > 0 and NUM_ORANGE_SQUARE > 0 and NUM_ORANGE_TRIANGLE > 0: # so it does NOT go out of the map
                if available_square(row, col):
                    mark_square(row, col, player)
                    if player == 1:
                        if dragged_type == 'circle':
                            NUM_ORANGE_CIRCLE -= 1
                        elif dragged_type == 'square':
                            NUM_ORANGE_SQUARE -= 1
                        elif dragged_type == 'triangle':
