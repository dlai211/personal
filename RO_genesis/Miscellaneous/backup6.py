...
'''
def minimax(board, depth, is_maximizing, pieces):
    ...
    if is_maximizing:
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                for size in ['S', 'M', 'L']:
                    if can_place_piece(row, col, size, 2, board, pieces['player']):
                        place_piece(row, col, size, 2, board, pieces['player'])
                        score = minimax(board, depth + 1, False, pieces)
                        remove_piece(row, col, board, pieces['player'], size)
                        best_score = max(best_score, score)
        return best_score
    else:
        best_score = float('inf')
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                for size in ['S', 'M', 'L']:
                    if can_place_piece(row, col, size, 1, board, pieces['player']):
                        place_piece(row, col, size, 1, board, pieces['player'])
                        score = minimax(board, depth + 1, True, pieces)
                        remove_piece(row, col, board, pieces['player'], size)
                        best_score = min(best_score, score)
        return best_score

available_pieces = {'L': 2, 'M': 3, 'S': 3}

def can_place_piece(row, col, size, player, board, available_pieces):
    board[row][col] = 0
    if available_pieces['L'] != 0 and available_pieces['M'] != 0 and available_pieces['S'] != 0:

    # Returns True if a piece of given size can be placed on the board at (row, col)
    current_player, current_size = board[row][col]
    if available_pieces[size] > 0 and (current_player is None or (current_player != player and size > current_size)):
        return True
    return False

def place_piece(row, col, size, player, board, available_pieces):
    board[row][col] = (player, size)
    available_pieces[size] -= 1

def remove_piece(row, col, board, available_pieces, size):
    board[row][col] = (None, None)  # or restore previous state if necessary
    available_pieces[size] += 1
...
player_game_pieces = [
    (WIDTH * 0.3, HEIGHT + 50, 'circle', 3),
    (WIDTH * 0.55, HEIGHT + 50, 'square', 2),
    (WIDTH * 0.8, HEIGHT + 50, 'triangle', 1)
]
ai_game_pieces = [
    (WIDTH * 0.3, HEIGHT + 150, 'circle', 3),
    (WIDTH * 0.55, HEIGHT + 150, 'square', 2),
    (WIDTH * 0.8, HEIGHT + 150, 'triangle', 1)
]


def check_collision(x, y, game_pieces):
    for idx, (cx, cy, type, size) in enumerate(game_pieces):
...
def piece_exists(game_pieces, new_piece):
    for piece in game_pieces:
        print('player', piece['player'], new_piece['player'])
        print('player', piece['size'], new_piece['size'])
        if ((piece['type'] == new_piece['type'] or piece['size'] > new_piece['size'])
                and piece['position'][0] == new_piece['position'][0]
                and piece['position'][1] == new_piece['position'][1]):
            print('NOT PASS')
            return True
    return False


def dragged_type2size(dragged_type):
    if dragged_type == 'circle':
        return 3
    if dragged_type == 'square':
        return 2
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
            ...
            if player == 1:  # Ensure only player can drag their pieces
                drag_index, dragged_type, dragged_size = check_collision(*pos, player_game_pieces)
                if drag_index is not None:
                    dragging = True
                    dragged_piece_index = drag_index
                    dragged_piece_origin = (player_game_pieces[drag_index][0],
                                            player_game_pieces[drag_index][1])  # Keep the original click position
        ...
        if event.type == pygame.MOUSEBUTTONUP and dragging:
            ...
            if col < 3 and row < 3 and not piece_exists(game_pieces,
                                                        {'type': dragged_type, 'size': dragged_type2size(dragged_type),
                                                         'player': 1,
                                                         'position': (col, row)}):  # so it does NOT go out of the map
                if dragged_type == 'circle' and NUM_ORANGE_CIRCLE > 0:
                    ...
                    NUM_ORANGE_CIRCLE -= 1
                    game_pieces.append({'type': dragged_type, 'size': 3, 'player': 1, 'position': (col, row)})

                else:
                    player_game_pieces[dragged_piece_index] = (
                    dragged_piece_origin[0], dragged_piece_origin[1], dragged_type, dragged_size)

                if dragged_type == 'square' and NUM_ORANGE_SQUARE > 0:
                    player_game_pieces, player, game_over = Mark(row, col, player, dragged_type, game_over)
                    NUM_ORANGE_SQUARE -= 1
                    game_pieces.append({'type': dragged_type, 'size': 2, 'player': 1, 'position': (col, row)})
                ...
                if dragged_type == 'triangle' and NUM_ORANGE_TRIANGLE > 0:
                    ...
                    game_pieces.append({'type': dragged_type, 'size': 1, 'player': 1, 'position': (col, row)})
        ...
        if event.type == pygame.MOUSEMOTION and dragging:
            ...
            player_game_pieces[drag_index] = (mouseX, mouseY, dragged_type, dragged_size)
